...
MongoClient.connect(connetToZeroHoneyMongoDb, function (err, client) {
    ...
    app.get('/getLastPretestData', function (req, res) {
        ...
        dbEccEvaluationData.collection('PreTest').find({ uid: id, smallCategory: category }, { projection: { date: 1 } }).toArray((err, result) => {
            ...
            validateYear(tempData, yearArray).then((tempData) => {
                if (tempData !== yearArray[0]) {
                    for (i = 0; i < monthArray.length; i++) {
                        if (i == 0) {

                            tempData = monthArray[0];
                        } else if (tempData < monthArray[i]) {
                            tempData = monthArray[i];
                        }

                    }

                } else {
                    dbEccEvaluationData.collection('PreTest').find({ date: { $eq: tempData } }, { projection: { date: 1 } }).toArray((err, result) => {
                        console.log(result);
    ...
    async function validateYear(tempData, yearArray) {
